...
let currentSessionMeta = null; // { quiz_id, title, topicName?, subtopic?, mode? }

const useremail = document.getElementById("userEmail");

// Content cache — avoids re-fetching on every click
const _contentCache = {
...
let _currentExercise = null; // { questions[] }
...
function injectExamSelectionStyles() {
  ...
  style.textContent = `
    ...
    .exam-card-icon {
      ...
      color: #0097b2; font-size: 1.3rem; margin-bottom: 4px;
    }
    ...
    .exam-loading {
      ...
      gap: 12px; padding: 48px; color: #9ca3af; font-size: 1rem;
    }
...
function setupEventListeners() {
  ...
  if (qaModeBtn) qaModeBtn.addEventListener("click", () => switchMode("qa"));
  ...
  // Prev/next topic buttons — handlers are set dynamically in _updateTopicNavFromAPI
...
async function loadContentSidebar() {
  ...
  if (!res?.success) {
    topicsNav.innerHTML = `<div style="padding:20px;color:#9ca3af;font-size:13px;">
      Failed to load topics.</div>`;
    showInfoPopup(
      "Oops!",
      res?.userMessage || "Could not load topics.",
...
async function loadContent(event, dataset) {
  ...
  try {
    ...
    // 2. Components for each section (parallel, cached)
    await Promise.all(
      sections.map(async (sec) => {
        ...
        try {
          ...
          if (!res?.success) {
            showInfoPopup(
              "Oops!",
              res?.userMessage || "Failed to load components.",
              "fas fa-exclamation-circle",
              "#e74c3c",
            );
...
function renderTabs(tabs) {
  return `<div class="format-tabs">
    <div class="tab-headers">${tabs
      .map(
        (tab, index) =>
          `<button class="tab-header ${index === 0 ? "active" : ""}" data-tab-index="${index}">${tab.title}</button>`,
      )
      .join("")}</div>
    ${tabs
      .map(
...
async function loadQuizSidebar() {
  ...
  try {
    ...
    if (!response?.success) {
      showInfoPopup(
        "Oops!",
        response?.userMessage || "Could not load topics.",
        "fas fa-exclamation-circle",
        "#e74c3c",
      );
      return;
    ...
    console.warn("Could not load topics for quiz sidebar:", err);
  }
}
...
async function fetchAndStartOverviewQuiz() {
  ...
  if (!response?.success) {
    ...
    showInfoPopup(
      "Oops!",
      response?.userMessage || "Failed to load quiz.",
      "fas fa-exclamation-circle",
      "#e74c3c",
    );
    return;
  ...
  const questions = await _fetchQuestionsById(questionIds);
  if (!questions.length) {
    showInfoPopup(
      "No questions available",
      "The overview quiz has no questions right now.",
...
async function _fetchQuestionsById(ids) {
  const results = await Promise.allSettled(
    ids.map((id) => FetchData(`/question/${id}`, true)),
  );
  const questions = results
    .filter((r) => r.status === "fulfilled" && r.value?.success !== false)
...
async function showExamSelectionScreen() {
  ...
  try {
    ...
    if (!response?.success) {
      ...
      showInfoPopup(
        "Oops!",
        response?.userMessage || "Failed to load exams.",
        "fas fa-exclamation-circle",
        "#e74c3c",
      );
      return;
    ...
    sessions.forEach((session) => {
      ...
      card.addEventListener("click", async () => {
        ...
        if (!res?.success) {
          ...
          showInfoPopup(
            "Oops!",
            res?.userMessage || "Failed to load exam.",
            "fas fa-exclamation-circle",
            "#e74c3c",
          );
          card.classList.remove("loading");
        ...
        confirmQuizNavigation(async () => {
          try {
            await loadAndStartQuiz(session.quiz_id, session.title, questionIds);
          } catch (err) {
...
async function loadAndStartQuiz(quizId, quizTitle, questionIds) {
  const questions = await _fetchQuestionsById(questionIds);
  if (!questions.length) {
    showInfoPopup(
      "No questions available",
