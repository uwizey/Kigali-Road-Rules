...
// ═════════════════════════════════════
// 🧪 MOCK API SIMULATION
// 📍 Defined here to replace backend
// ═════════════════════════════════════

const MOCK_API_DELAY_MIN = 200;
const MOCK_API_DELAY_MAX = 500;
...
const MOCK_API = {
  ...
  topics: [
    ...
    },
    {
      ...
      name: "Road Rules",
      subtopics: [
        { id: 201, name: "Right of Way" },
        { id: 202, name: "Parking Rules" },
      ],
    },
    {
      id: 3,
      name: "Safe Driving",
      subtopics: [
        { id: 301, name: "Defensive Driving" },
        { id: 302, name: "Night Driving" },
      ],
    },
  ],
  sectionsByTopic: {
    ...
    202: [
      {
        section_id: 405,
        topic_id: 202,
        title: "Parking Distances",
        order_index: 0,
        is_locked: false,
        created_at: "2026-04-04T11:15:00Z",
      },
    ],
    301: [
      {
    ...
    401: [
      {
        component_id: 901,
        section_id: 401,
        title: "Sharp Curve Signs",
        format_type: "flipcards",
        component_type: "flipcards",
        order_index: 0,
        ...
        items: [
          {
            ...
            content: "Reduce speed and position the vehicle before entering the bend.",
            order_index: 0,
            image:
          ...
          },
          {
            item_id: 1502,
            title: "Double Bend",
            content: "Prepare for two consecutive curves and avoid overtaking.",
            order_index: 1,
            image:
      ...
      },
      {
        ...
        items: [
          {
            item_id: 1503,
            title: "Before the sign",
            content: "Ease off the accelerator early so braking is smooth and predictable.",
            order_index: 0,
          },
          {
            item_id: 1504,
            title: "During poor weather",
            content: "Increase your margin further when visibility or grip is reduced.",
            order_index: 1,
          },
        ],
      },
        ...
        items: [
          {
            ...
            content: "Road work signs usually appear well before the actual work zone.",
            order_index: 0,
          },
          {
            item_id: 1506,
            title: "Merge carefully",
            content: "Follow cones and lane arrows, then zipper into the open lane.",
            order_index: 1,
          },
        ],
      },
    ...
    403: [
      {
        component_id: 904,
        section_id: 403,
        title: "Mandatory Sign Layout",
        format_type: "image_right",
        component_type: "image_right",
        order_index: 0,
        created_at: "2026-04-06T10:00:00Z",
        items: [
          {
            item_id: 1507,
            title: "Blue Circular Signs",
            content: "These signs tell the driver what action must be taken, such as keeping left.",
            order_index: 0,
            image:
              "https://images.unsplash.com/photo-1516733968668-dbdce39c4651?auto=format&fit=crop&w=600&q=80",
          },
        ],
      },
    ],
        ...
        items: [
          {
            ...
            content: "Yield to vehicles already in the intersection and check both directions.",
            order_index: 0,
          },
          {
            item_id: 1509,
            title: "Roundabout",
            content: "Give way to traffic already circulating unless road signs state otherwise.",
            order_index: 1,
          },
        ],
      },
    ...
    405: [
      {
        component_id: 906,
        section_id: 405,
        title: "Minimum Parking Gaps",
        format_type: "image_overlay",
        component_type: "image_overlay",
        order_index: 0,
        created_at: "2026-04-06T12:00:00Z",
        items: [
          {
            item_id: 1510,
            title: "Keep Clear Zones",
            content: "Stay clear of corners, crossings, and bus stops to preserve sight lines.",
            order_index: 0,
            image:
    ...
    406: [
      {
        component_id: 907,
        section_id: 406,
        title: "Visual Scan Routine",
        format_type: "exercise",
...
function mockWait() {
  const duration =
    Math.floor(Math.random() * (MOCK_API_DELAY_MAX - MOCK_API_DELAY_MIN + 1)) +
    MOCK_API_DELAY_MIN;
...
function mockResponse(success, status, data, userMessage) {
  return {
    success,
    status,
    data: data ?? null,
...
function getTopicSections(topicId) {
  if (!MOCK_API.sectionsByTopic[topicId]) MOCK_API.sectionsByTopic[topicId] = [];
  return MOCK_API.sectionsByTopic[topicId];
}

function getSectionComponents(sectionId) {
  if (!MOCK_API.componentsBySection[sectionId]) {
    MOCK_API.componentsBySection[sectionId] = [];
  }
  return MOCK_API.componentsBySection[sectionId];
}

function findComponent(componentId) {
  for (const [sectionId, components] of Object.entries(MOCK_API.componentsBySection)) {
    const component = components.find((entry) => entry.component_id === componentId);
    if (component) {
      return { component, sectionId: Number(sectionId), components };
    }
  }
  return null;
}
...
async function buildStoredItems(rawItems, files) {
  const stored = [];
  for (let index = 0; index < rawItems.length; index += 1) {
    const raw = rawItems[index] || {};
    const nextItem = {
      item_id: raw.item_id ? Number(raw.item_id) : MOCK_API.nextItemId++,
    ...
    if (raw.image === "__keep__") {
      nextItem.image = raw.existing_image || null;
    } else if (raw.image === null) {
      nextItem.image = null;
    } else if (typeof raw.image === "string" && files?.has(raw.image)) {
      nextItem.image = await readFileAsDataUrl(files.get(raw.image));
    } else if (typeof raw.image === "string" && raw.image) {
      nextItem.image = raw.image;
    } else {
      nextItem.image = raw.image ?? null;
    }

    stored.push(nextItem);
  }
  return stored;
}

async function parseComponentPayload(payload, existingComponent = null) {
  ...
  if (isForm) {
    ...
    const rawItems = JSON.parse(payload.get("items") || "[]");
    const existingImages = new Map(
      (existingComponent?.items || []).map((item) => [Number(item.item_id), item.image || null]),
    );

    base = {
      title: payload.get("title") || existingComponent?.title || "Untitled Component",
      format_type:
        payload.get("format_type") ||
        existingComponent?.format_type ||
        "",
      component_type:
        payload.get("format_type") ||
        existingComponent?.format_type ||
        "",
      order_index: Number(
        payload.get("order_index") ?? existingComponent?.order_index ?? 0,
      ),
      section_id: Number(
        payload.get("section_id") ?? existingComponent?.section_id ?? 0,
      ),
      items: rawItems.map((item, idx) => ({
        ...item,
        order_index: item.order_index ?? idx,
        existing_image: item.item_id
          ? existingImages.get(Number(item.item_id)) || null
          : null,
      })),
      deleted_item_ids: JSON.parse(payload.get("deleted_item_ids") || "[]"),
    };
  } else {
    base = {
      title: payload?.title || existingComponent?.title || "Untitled Component",
      format_type:
        payload?.format_type ??
      ...
      component_type:
        ...
        "",
      order_index: Number(
        payload?.order_index ?? existingComponent?.order_index ?? 0,
      ),
      section_id: Number(
        payload?.section_id ?? existingComponent?.section_id ?? 0,
      ),
      items: Array.isArray(payload?.items) ? payload.items : existingComponent?.items || [],
      deleted_item_ids: Array.isArray(payload?.deleted_item_ids)
        ? payload.deleted_item_ids
        : [],
    };
  }

  const deletedIds = new Set((base.deleted_item_ids || []).map(Number));
  const filteredExisting =
    existingComponent?.items?.filter((item) => !deletedIds.has(Number(item.item_id))) || [];

  const mergedItemsSource =
    base.items.length > 0
      ? base.items
      : base.format_type === "exercise"
        ? []
        : filteredExisting;

  const storedItems = await buildStoredItems(mergedItemsSource, files);
...
// ══════════════════════════════════════════════
// 🧪 MOCK API SIMULATION (ADMIN CONTENT FILE)
// 📍 LOCATION: Right below imports
// ══════════════════════════════════════════════
...
// ── FETCH ─────────────────────────────────────
async function FetchData(endpoint, auth = true) {
  await mockWait();

  try {
    const { clean, parts } = parseEndpoint(endpoint);

    if (clean === "/topic") {
      return mockResponse(true, 200, { topics: cloneMock(MOCK_API.topics) });
    }

    if (parts[0] === "sections" && parts.length === 2) {
      const topicId = Number(parts[1]);
      return mockResponse(true, 200, {
        sections: cloneMock(getTopicSections(topicId)).sort(
          (a, b) => (a.order_index ?? 0) - (b.order_index ?? 0),
        ),
      });
    }

    if (parts[0] === "sections" && parts[2] === "components") {
      const sectionId = Number(parts[1]);
      return mockResponse(true, 200, {
        section_id: sectionId,
        components: cloneMock(getSectionComponents(sectionId)).sort(
          (a, b) => (a.order_index ?? 0) - (b.order_index ?? 0),
        ),
      });
    }
    ...
    if (parts[0] === "component" && parts.length === 2) {
      ...
      if (!found) {
        return mockResponse(false, 404, null, "Component not found.");
      }
      return mockResponse(true, 200, {
        component: cloneMock(found.component),
      });
    }

    return mockResponse(false, 404, null, `Unknown endpoint: ${clean}`);
  } catch (err) {
    console.error("[Mock FetchData]", err);
    return mockResponse(false, 500, null, "Mock server error while loading data.");
  }
}

// ── POST ──────────────────────────────────────
async function PostData(endpoint, payload, auth = true) {
  await mockWait();

  try {
    const { clean } = parseEndpoint(endpoint);

    if (clean === "/section") {
      const topicId = Number(payload?.topic_id);
      if (!topicId) {
        return mockResponse(false, 400, null, "A topic_id is required to create a section.");
      }

      const section = {
        section_id: MOCK_API.nextSectionId++,
        topic_id: topicId,
        title: payload?.title || "Untitled Section",
        order_index: Number(payload?.order_index ?? getTopicSections(topicId).length),
        is_locked: Boolean(payload?.is_locked),
        created_at: new Date().toISOString(),
      };

      getTopicSections(topicId).push(section);
      return mockResponse(true, 201, {
        status: true,
        section: cloneMock(section),
      });
    }

    if (clean === "/component") {
      const component = {
        component_id: MOCK_API.nextComponentId++,
        section_id: Number(payload?.section_id),
        title: payload?.title || "New Component",
        format_type: payload?.format_type || payload?.component_type || "",
        component_type: payload?.format_type || payload?.component_type || "",
        order_index: Number(
          payload?.order_index ??
            getSectionComponents(Number(payload?.section_id)).length,
        ),
        created_at: new Date().toISOString(),
        items: [],
      };

      if (!component.section_id) {
        return mockResponse(false, 400, null, "A section_id is required to create a component.");
      }

      getSectionComponents(component.section_id).push(component);
      return mockResponse(true, 201, {
        status: true,
        component: cloneMock(component),
      });
    }

    return mockResponse(false, 404, null, `Unknown endpoint: ${clean}`);
  } catch (err) {
    console.error("[Mock PostData]", err);
    return mockResponse(false, 500, null, "Mock server error while creating data.");
  }
}

// ── UPDATE ────────────────────────────────────
async function UpdateData(endpoint, payload, auth = true) {
  await mockWait();
  ...
  try {
    ...
    if (parts[0] === "sections" && parts.length === 2) {
      ...
      Object.values(MOCK_API.sectionsByTopic).some((sections) => {
        targetSection =
          sections.find((entry) => entry.section_id === sectionId) || null;
        return Boolean(targetSection);
      });

      if (!targetSection) {
        return mockResponse(false, 404, null, "Section not found.");
      }

      targetSection.title = payload?.title ?? targetSection.title;
      targetSection.order_index = Number(
        payload?.order_index ?? targetSection.order_index ?? 0,
      );
      targetSection.is_locked = Boolean(
        payload?.is_locked ?? targetSection.is_locked,
      );

      return mockResponse(true, 200, {
        status: true,
        section: cloneMock(targetSection),
      });
    }
    ...
    if (parts[0] === "component" && parts.length === 2) {
      ...
      const updated = await parseComponentPayload(payload, found.component);
      Object.assign(found.component, updated, {
        component_id: componentId,
        updated_at: new Date().toISOString(),
      });

      found.components.sort((a, b) => (a.order_index ?? 0) - (b.order_index ?? 0));
    ...
    console.error("[Mock UpdateData]", err);
    return mockResponse(false, 500, null, "Mock server error while updating data.");
  }
}
...
async function DeleteData(endpoint, auth = true) {
  ...
  try {
    ...
    if (parts[0] === "sections" && parts.length === 2) {
      const sectionId = Number(parts[1]);
      for (const topicId of Object.keys(MOCK_API.sectionsByTopic)) {
        const sections = getTopicSections(Number(topicId));
        const index = sections.findIndex((entry) => entry.section_id === sectionId);
        if (index !== -1) {
          sections.splice(index, 1);
          delete MOCK_API.componentsBySection[sectionId];
          return mockResponse(true, 200, { status: true });
        }
      }
      return mockResponse(false, 404, null, "Section not found.");
    }
    ...
    if (parts[0] === "component" && parts.length === 2) {
      ...
      const index = found.components.findIndex(
        (entry) => entry.component_id === componentId,
      );
      found.components.splice(index, 1);
      return mockResponse(true, 200, { status: true });
    }

    return mockResponse(false, 404, null, `Unknown endpoint: ${clean}`);
  } catch (err) {
    console.error("[Mock DeleteData]", err);
    return mockResponse(false, 500, null, "Mock server error while deleting data.");
  }
}
/**
