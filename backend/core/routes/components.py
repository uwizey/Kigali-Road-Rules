...
def create_component():
    try:
        ...
        new_component = Component(
            section_id=data["section_id"], title=data.get("title", "Untitled Component"),
            component_type=data.get("format_type", "content"), order_index=data.get("order_index", 0)
        )
        ...
        for idx, item in enumerate(data.get("items", [])):
            ...
            db.session.add(ComponentItem(
                component_id=new_component.component_id, format_type=item.get("format_type", "content"),
                title=item.get("title", f"Item {idx+1}"), content=item.get("content", None),
                order_index=item.get("order_index", idx), mimetype=item.get("mimetype", None),
                image_data=item.get("image_data", None)
            ))
        db.session.commit()
        return jsonify({"status": True, "message": "Component created successfully", "component_id": new_component.component_id}), 201
    except Exception as e:
...
def update_component(component_id):
    try:
        ...
        comp.title = data.get("title", comp.title)
        ...
        for idx, item_data in enumerate(json.loads(data.get("items", "[]"))):
            ...
            if item_id:
                ...
                new_item = ComponentItem(
                    component_id=comp.component_id, title=item_data.get("title", f"Item {idx+1}"),
                    content=item_data.get("content", None), format_type=item_data.get("format_type", "content"),
        ...
        db.session.commit()
        return jsonify({"status": True, "message": "Component updated successfully",
                        "component_id": comp.component_id, "title": comp.title}), 200
    except Exception as e:
